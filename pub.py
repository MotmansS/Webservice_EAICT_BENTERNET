...
class Playr:
    def __init__(self, name, score, DealerScore, randomkaart, randomdealerkaart, pcontinue, computerhand, playerhand,
                 bufferhand):
        self.name = name
        self.score = score
	...
	if Player.name != 'y' and Player.name != 'n':
		...
		for i in range(9):
			...
			if Player.pcontinue == 'y':
				...
				message = "BlackJackService>Player>{name}>PlayerHand>{hand}".format(name=Player.name,
				...
				message = "BlackJackService>Player>{name}>ComputerHand>{hand}".format(name=Player.name,
																					  hand=Player.computerhand)
				print(message)
				PubSock.send_string(message)
				print("we got to Play")
			elif Player.pcontinue == 'n':
				if (Player.score < 22 and Player.score > Player.DealerScore) or (
						Player.score < 22 and Player.DealerScore > 22) or (Player.score > Player.DealerScore):
...
while True:
    ...
    for Player in playerlist:
        t = threading.Thread(mainFunct(Player))
        t.start()